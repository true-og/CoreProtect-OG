...
public class SpigotHandler extends SpigotAdapter implements SpigotInterface {
        ...
        try {
            ...
            if (Config.getGlobal().HOVER_EVENTS) {
                ...
                if (tooltipText.contains(Color.MAGIC)) {
                    ...
                    // to-do
                    /*
                     * ComponentBuilder formattedComponent = new ComponentBuilder(); StringBuilder
                     * messageTest = new StringBuilder(); String colorChar =
                     * String.valueOf(ChatColor.COLOR_CHAR); boolean isObfuscated = false;
                     * 
                     * String[] tooltip = tooltipText.split(colorChar); for (String splitText :
                     * tooltip) { boolean setObfuscated = splitText.startsWith("k"); splitText =
                     * setObfuscated ? splitText.substring(1) : (splitText.length() > 0 ? colorChar
                     * : "") + splitText; if ((setObfuscated && !isObfuscated) || (!setObfuscated &&
                     * isObfuscated)) {
                     * formattedComponent.append(TextComponent.fromLegacyText(processComponent(
                     * messageTest.toString()))); formattedComponent.obfuscated(false); //
                     * setObfuscated
                     * formattedComponent.append(TextComponent.fromLegacyText(processComponent(
                     * splitText))); messageTest.setLength(0); isObfuscated = !isObfuscated; } else
                     * { messageTest.append(splitText); } }
                     * 
                     * if (messageTest.length() > 0) {
                     * formattedComponent.append(TextComponent.fromLegacyText(processComponent(
                     * messageTest.toString()))); }
                     * 
    ...
    public void setHoverEvent(Object component, String text) {
        ...
        if (Config.getGlobal().HOVER_EVENTS) {
            ...
            ((TextComponent) component).setHoverEvent(
