...
public class Config extends Language {
	...
	private boolean getBoolean(final String key) {
		...
		return configured != null && StringUtils.startsWith(configured, "t");
	...
	private boolean getBoolean(final String key, final boolean dfl) {
		...
		final String configured = this.get(key, null);
		return configured == null ? dfl : StringUtils.startsWith(configured, "t");
	...
	private int getInt(final String key, final int dfl) {
		...
		return StringUtils.isEmpty(configured) ? dfl : Integer.parseInt(configured);
	...
	private static Map<String, byte[]> loadFiles(String fileName) throws IOException {
		...
		if (!globalFile.exists() || globalFile.length() == 0) {
			...
			try (final InputStream in = plugin.getResource(fileName)) {
				...
				Files.copy(in, globalFile.toPath(), StandardCopyOption.REPLACE_EXISTING);
		...
		final File[] files = configFolder.listFiles((File file) -> StringUtils.endsWith(file.getName(), ".yml"));
		if (files != null) {

			for (final File worldConfigFile : files) {
				...
				map.put(StringUtils.substring(name, 0, name.length() - ".yml".length()),
