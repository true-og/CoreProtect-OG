            ...
            if (inventories.get(id) != null) {
                ...
                if (ConfigHandler.loggingChest.get(loggingChestId) != null) {
                    ...
                    if (ConfigHandler.forceContainer.get(loggingChestId) != null) {
                        force_size =
                                ConfigHandler.forceContainer.get(loggingChestId).size();
                    }
                    if (current_chest == forceData
                            || force_size > 0) { // This prevents client side chest sorting mods from messing things up.
                        ContainerLogger.log(
                                preparedStmtContainer, preparedStmtItems, batchCount, user, type, inventory, location);
