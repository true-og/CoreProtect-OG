...
public final class ProjectileLaunchListener extends Queue implements Listener {
    ...
    public static void playerLaunchProjectile(Location location, String user, ItemStack itemStack, int amount,
            int delay, int offset, int action)
    {
    ...
    protected void onProjectileLaunch(ProjectileLaunchEvent event) {
        ...
        while (it.hasNext()) {
            ...
            if ((data[1].equals(key) || data[2].equals(key)) && (entityMaterial == itemStack.getType()
    ...
    /*
     * @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
     * protected void onEntityShootBow(EntityShootBowEvent event) { if
     * (!(event.getEntity() instanceof Player)) { return; }
     * 
     * ItemStack itemStack = event.getConsumable();
     * playerLaunchProjectile(event.getEntity().getLocation(),
     * event.getEntity().getName(), itemStack, 1, 1, 0, ItemLogger.ITEM_SHOOT); }
     */
    // Requires Bukkit 1.16+
    /*
     * @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
     * protected void onPlayerEggThrow(PlayerEggThrowEvent event) { Egg egg =
     * event.getEgg(); ItemStack itemStack = egg.getItem();
     * playerLaunchProjectile(event.getPlayer().getLocation(),
     * event.getPlayer().getName(), itemStack, 0, 1, 0, ItemLogger.ITEM_THROW); }
     */
