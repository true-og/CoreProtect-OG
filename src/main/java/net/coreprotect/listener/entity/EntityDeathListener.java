...
public final class EntityDeathListener extends Queue implements Listener {
    ...
    protected static void logEntityDeath(LivingEntity entity, String e) {
        ...
        boolean isCommand = (damage.getCause() == DamageCause.VOID
                && entity.getLocation().getBlockY() >= BukkitAdapter.ADAPTER.getMinHeight(entity.getWorld()));
        if (e == null) {
        ...
        List<DamageCause> validDamageCauses = Arrays.asList(DamageCause.SUICIDE, DamageCause.POISON, DamageCause.THORNS,
                DamageCause.MAGIC, DamageCause.WITHER);
        ...
        if (entity instanceof ArmorStand) {
            ...
            /*
             * else if (isCommand) { entityLocation.setY(entityLocation.getY() + 0.99);
             * Block block = entityLocation.getBlock(); Database.containerBreakCheck(e,
             * Material.ARMOR_STAND, entity, null, block.getLocation());
             * Queue.queueBlockBreak(e, block.getState(), Material.ARMOR_STAND, null, (int)
