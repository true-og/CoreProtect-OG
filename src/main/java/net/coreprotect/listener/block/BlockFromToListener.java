        ...
        if (!event.isCancelled()) {
                    ...
                    || (Config.getConfig(world).LAVA_FLOW && type.equals(Material.LAVA)))
            {
                ...
                if (Config.getConfig(block.getWorld()).BLOCK_BREAK) {
                    ...
                    Queue.queueBlockBreak(user, block.getState(), block.getType(), block.getBlockData().getAsString(),
                            0);
                ...
                if (Config.getConfig(block.getWorld()).BLOCK_PLACE) {
                    ...
                    Queue.queueBlockPlace(user, toBlockState, block.getType(), toBlockState, type, -1, 0,
                            blockData.getAsString());
