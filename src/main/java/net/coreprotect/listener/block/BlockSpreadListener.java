...
public final class BlockSpreadListener extends Queue implements Listener {
    ...
    protected void onBlockSpread(BlockSpreadEvent event) {
        ...
        if (Config.getConfig(event.getBlock().getWorld()).VINE_GROWTH && (BlockGroup.VINES.contains(type)
                || BlockGroup.AMETHYST.contains(type) || type == Material.CHORUS_FLOWER || type == Material.BAMBOO))
        {
            ...
            if (BlockGroup.VINES.contains(type)) {
                ...
                Queue.queueBlockPlaceDelayed("#bamboo", sourceBlock.getLocation(), type, null, sourceBlock.getState(),
                        0);
            ...
            queueBlockPlace("#sculk_catalyst", block.getState(), block.getType(), block.getState(), type, -1, 0,
                    blockstate.getBlockData().getAsString());
    ...
    private boolean checkCacheData(Block block, Material type) {
        ...
        String cacheId = block.getX() + "." + block.getY() + "." + block.getZ() + "."
                + Util.getWorldId(block.getWorld().getName());
