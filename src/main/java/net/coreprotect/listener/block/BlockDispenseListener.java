        ...
        if (!event.isCancelled() && Config.getConfig(world).BLOCK_PLACE) {
            ...
            if (item != null && blockData instanceof Dispenser) {
                        ...
                        || (!BlockPreDispenseListener.useForDroppers && block.getType() == Material.DROPPER))
                {
                    ...
                    if (dispenseRelative || material.equals(Material.FLINT_AND_STEEL)
                ...
                if (!type.equals(Material.AIR) || !newBlock.getType().equals(Material.AIR)) {
                    ...
                    if (type == Material.FIRE) { // lit a lightable block
                        ...
                        if (!type.equals(Material.AIR)) {
                            ...
                            queueBlockPlace(user, newBlock.getState(), newBlock.getType(), blockState, type, 1, 1,
                                    null);

                        } else {

                            Queue.queueBlockBreak(user, newBlock.getState(), newBlock.getType(),
                                    newBlock.getBlockData().getAsString(), 0);
