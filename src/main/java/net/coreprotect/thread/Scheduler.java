...
public class Scheduler {
    ...
    public static Object scheduleSyncRepeatingTask(CoreProtect plugin, Runnable task, Object regionData, int delay,
            int period)
    {
        ...
        if (ConfigHandler.isFolia) {
            ...
            if (regionData instanceof Location) {
                ...
                Location location = (Location) regionData;
                return Bukkit.getServer().getRegionScheduler().runAtFixedRate(plugin, location, value -> task.run(),
                        delay, period);
                ...
                return Bukkit.getServer().getGlobalRegionScheduler().runAtFixedRate(plugin, value -> task.run(), delay,
                        period);
    ...
    public static void scheduleAsyncDelayedTask(CoreProtect plugin, Runnable task, int delay) {
        ...
        if (ConfigHandler.isFolia) {
            ...
            if (delay == 0) {
                ...
                Bukkit.getServer().getAsyncScheduler().runDelayed(plugin, value -> task.run(), (delay * 50L),
