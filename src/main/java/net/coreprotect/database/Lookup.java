...
public class Lookup extends Queue {
    ...
    public static long countLookupRows(Statement statement, CommandSender user, List<String> checkUuids,
            List<String> checkUsers, List<Object> restrictList, Map<Object, Boolean> excludeList,
            List<String> excludeUserList, List<Integer> actionList, Location location, Integer[] radius, Long[] rowData,
            long startTime, long endTime, boolean restrictWorld, boolean lookup)
    {
    ...
    public static List<String[]> performLookup(Statement statement, CommandSender user, List<String> checkUuids,
            List<String> checkUsers, List<Object> restrictList, Map<Object, Boolean> excludeList,
            List<String> excludeUserList, List<Integer> actionList, Location location, Integer[] radius, long startTime,
            long endTime, boolean restrictWorld, boolean lookup)
    {
        ...
        try {
            ...
            List<Object[]> lookupList = performLookupRaw(statement, user, checkUuids, checkUsers, restrictList,
                    excludeList, excludeUserList, actionList, location, radius, null, startTime, endTime, -1, -1,
    ...
    protected static List<Object[]> performLookupRaw(Statement statement, CommandSender user, List<String> checkUuids,
            List<String> checkUsers, List<Object> restrictList, Map<Object, Boolean> excludeList,
            List<String> excludeUserList, List<Integer> actionList, Location location, Integer[] radius, Long[] rowData,
            long startTime, long endTime, int limitOffset, int limitCount, boolean restrictWorld, boolean lookup)
    {
        ...
        try {
            ...
            ResultSet results = rawLookupResultSet(statement, user, checkUuids, checkUsers, restrictList, excludeList,
                    excludeUserList, actionList, location, radius, rowData, startTime, endTime, limitOffset, limitCount,
                    restrictWorld, lookup, false);
            ...
            while (results.next()) {
                ...
                if (actionList.contains(6) || actionList.contains(7)) {
                    ...
                    if (PluginChannelHandshakeListener.getInstance().isPluginChannelPlayer(user)) {
                        ...
                        dataArray = new Object[] { resultId, resultTime, resultUserId, resultMessage, resultWorldId,
                    ...
                    if (valid) {
                        ...
                        if (hasTbl) {
                            ...
                            Object[] dataArray = new Object[] { resultId, resultTime, resultUserId, resultX, resultY,
                                    ...
                                    resultAmount, resultMeta, resultBlockData, resultTable };
                            list.add(dataArray);
            ...
            long startTime, long endTime, int limitOffset, int limitCount, boolean restrictWorld, boolean lookup)
    {
        ...
        try {
            ...
            List<Object[]> lookupList = performLookupRaw(statement, user, checkUuids, checkUsers, restrictList,
                    excludeList, excludeUserList, actionList, location, radius, rowData, startTime, endTime,
                    limitOffset, limitCount, restrictWorld, lookup);
            newList = convertRawLookup(statement, lookupList);
            ...
            boolean count)
    {
        ...
        try {
            ...
            } else if (inventoryQuery || actionExclude.length() > 0 || includeBlock.length() > 0
                    || includeEntity.length() > 0 || excludeBlock.length() > 0 || excludeEntity.length() > 0)
            {
            ...
            if ((lookup && actionList.size() == 0) || (itemLookup && !actionList.contains(0))) {
                ...
                query = unionSelect + "SELECT " + "'0' as tbl," + rows + " FROM " + ConfigHandler.prefix + "block "
                        + index + "WHERE" + baseQuery + unionLimit + ") UNION ALL ";
                itemLookup = true;
            ...
            if (itemLookup) {
                ...
                query = query + unionSelect + "SELECT " + "'1' as tbl," + rows + " FROM " + ConfigHandler.prefix
                        + "container WHERE" + queryBlock + unionLimit + ") UNION ALL ";
                ...
                query = query + unionSelect + "SELECT " + "'2' as tbl," + rows + " FROM " + ConfigHandler.prefix
                        + "item WHERE" + queryBlock + unionLimit + ")";
            ...
            if (query.length() == 0) {
                ...
                query = "SELECT " + "'0' as tbl," + rows + " FROM " + ConfigHandler.prefix + queryTable + " " + index
                        + "WHERE" + baseQuery;
    ...
    /*
     * private static long calculateTableOffset(long col2, long col3, long
     * limitOffset, int limitCount) { return (limitOffset - (col2 + col3)) < 0L ? 0L
     * : (limitOffset - (col2 + col3)); }
     * 
     * private static long calculateTableLimit(long col1, long col2, long col3, long
     * limitOffset, int limitCount) { long offset = calculateTableOffset(col2, col3,
     * limitOffset, limitCount); long limit = (col2 + col3) + limitCount; limit =
    ...
    public static String whoPlaced(Statement statement, BlockState block) {
        ...
        try {
            ...
            String query = "SELECT user,type FROM " + ConfigHandler.prefix + "block " + Util.getWidIndex("block")
