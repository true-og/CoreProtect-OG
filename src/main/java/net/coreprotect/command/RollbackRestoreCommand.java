...
public class RollbackRestoreCommand {
    ...
    protected static void runCommand(CommandSender player, Command command, boolean permission, String[] args,
            Location argLocation, long forceStart, long forceEnd)
    {
        ...
        if (permission) {
            ...
            if ((player instanceof Player || player instanceof BlockCommandSender) && argRadius == null
                    && DEFAULT_RADIUS > 0 && !forceglobal && !argAction.contains(11))
            {
            ...
            } else if (!argAction.contains(4) && Config.getGlobal().EXCLUDE_TNT && !argExclude.containsKey(Material.TNT)
                ...
                if (argAction.size() > 0) {
                    ...
                    if (argAction.contains(4)) {
                        ...
                        if (argUsers.contains("#global") || (argUsers.size() == 0 && argRadius == null)) {
                            ...
                            Chat.sendMessage(player,
                                    Color.DARK_AQUA + "CoreProtect " + Color.WHITE + "- "
                                            + Phrase.build(Phrase.PREVIEW_TRANSACTION,
                    ...
                    if (argAction.contains(8) || (argAction.contains(11) && !argAction.contains(4))
                            || (!argAction.contains(0) && !argAction.contains(1) && !argAction.contains(3)
                                        ...
                                        if (exists) {
                                            ...
                                            if (finalArgAction.contains(5)) {
                                                ...
                                                ContainerRollback.performContainerRollbackRestore(statement, player2,
                                                        ...
                                                        finalArgAction, location, radius, finalTimeStart, finalTimeEnd,
                                                        restrictWorld, false, verbose, action);

                                            } else {

                                                Rollback.performRollbackRestore(statement, player2, uuidList,
                                                        rollbackusers2, rtime, blist, elist, euserlist, finalArgAction,
                                                        location, radius, finalTimeStart, finalTimeEnd, restrictWorld,
                                                        false, verbose, action, finalPreview);
