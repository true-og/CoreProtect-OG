                ...
                public void run() {
                    try (Connection connection = Database.getConnection(true)) {
                        ConfigHandler.lookupThrottle.put(
                                player2.getName(), new Object[] {true, System.currentTimeMillis()});
                        if (connection != null) {
                            Statement statement = connection.createStatement();
                            List<String> blockData = ChestTransactionLookup.performLookup(
                                    command.getName(), statement, location, player2, p2, finalLimit, false);
                ...
                public void run() {
                    try (Connection connection = Database.getConnection(true)) {
                        ...
                        if (connection != null) {
                            Statement statement = connection.createStatement();
                            if (t == 8) {
                                List<String> signData = SignMessageLookup.performLookup(
                                        command.getName(),
                                        statement,
                                        fblockstate.getLocation(),
                                        player2,
                                        p2,
                                        finalLimit);
                                ...
                                if (t == 7) {
                                    blockdata = InteractionLookup.performLookup(
                                            command.getName(), statement, fblock, player2, 0, p2, finalLimit);
                                } else {
                                    blockdata = BlockLookup.performLookup(
                                            command.getName(), statement, fblockstate, player2, 0, p2, finalLimit);
                                }
                                    ...
                                    for (String check : rollbackusers2) {
                                        if ((!check.equals("#global") && !check.equals("#container"))
                                            ...
                                            if (!exists) {
                                                ...
                                                if (ConfigHandler.uuidCache.get(check.toLowerCase(Locale.ROOT))
                                                        != null) {
                                                    String uuid =
                                                            ConfigHandler.uuidCache.get(check.toLowerCase(Locale.ROOT));
                                                    uuidList.add(uuid);
                                                }
                                            }
                                        } else {
                                    ...
                                    if (exists) {
                                        ...
                                        if (finalArgWid > 0) {
                                            ...
                                            location = new Location(
                                                    Bukkit.getServer().getWorld(Util.getWorldName(finalArgWid)),
                                                    finalX,
                                                    finalY,
                                                    finalZ);
                                        } else if (location != null) {
                                            location = new Location(
                                                    Bukkit.getServer().getWorld(Util.getWorldName(finalWid)),
                                                    finalX,
                                                    finalY,
                                                    finalZ);
                                        }
                                        ...
                                        if (finalCount) {
                                            String row_format =
                                                    NumberFormat.getInstance().format(rows);
                                            Chat.sendMessage(
                                                    player2,
                                                    Color.DARK_AQUA + "CoreProtect " + Color.WHITE + "- "
                                                            + Phrase.build(
                                            ...
                                            List<String[]> lookupList = Lookup.performPartialLookup(
                                                    statement,
                                                    player2,
                                                    uuidList,
                                                    userList,
                                            ...
                                            Chat.sendMessage(
                                                    ...
                                                    Color.WHITE + "----- " + Color.DARK_AQUA
                                                            + Phrase.build(
                                                                    ...
                                                                    "CoreProtect" + Color.WHITE + " | "
                                                                            + Color.DARK_AQUA)
                                                            + Color.WHITE + " -----");
                                            if (finalArgAction.contains(6)
                                                    || finalArgAction.contains(7)) { // Chat/command
                                                for (String[] data : lookupList) {
                                                    String time = data[0];
                                                    String dplayer = data[1];
                                                    String message = data[2];
                                                    String timeago = Util.getTimeSince(
                                                            Integer.parseInt(time), unixtimestamp, true);
                                                    Chat.sendComponent(
                                                            player2,
                                                            timeago + " " + Color.WHITE + "- " + Color.DARK_AQUA
                                                                    + dplayer + ": " + Color.WHITE,
                                                            message);
                                                ...
                                                for (String[] data : lookupList) {
                                                    ...
                                                    Chat.sendComponent(
                                                            ...
                                                            Color.WHITE + leftPadding + Color.GREY + "^ "
                                                                    + Util.getCoordinates(
                                                                            command.getName(), wid, x, y, z, true, true)
                                                                    + "");
                                                    PluginChannelListener.getInstance()
                                                            .sendInfoData(
                                                                    player2,
                                                ...
                                                for (String[] data : lookupList) {
                                                    ...
                                                    PluginChannelListener.getInstance()
                                                            .sendUsernameData(
                                                                    player2, Integer.parseInt(time), user, username);
                                                }
                                            ...
                                            } else if (finalArgAction.contains(4)
                                                    && finalArgAction.contains(11)) { // inventory transactions
                                                for (String[] data : lookupList) {
                                                    String time = data[0];
                                                    String dplayer = data[1];
                                                    ...
                                                    String rbd = ((Integer.parseInt(data[8]) == 2
                                                                    || Integer.parseInt(data[8]) == 3)
                                                            ? Color.STRIKETHROUGH
                                                            : "");
                                                    String timeago = Util.getTimeSince(
                                                            Integer.parseInt(time), unixtimestamp, true);
                                                    Material blockType = Util.itemFilter(
                                                            Util.getType(dtype), (Integer.parseInt(data[13]) == 0));
                                                    String dname = Util.nameFilter(
                                                            blockType.name().toLowerCase(Locale.ROOT), ddata);
                                                    byte[] metadata = data[11] == null
                                                            ? null
                                                    ...
                                                    if (daction == 2 || daction == 3) { // LOOKUP_ITEM
                                                        ...
                                                        selector = (daction == ItemLogger.ITEM_CREATE
                                                                ? Selector.FIRST
                                                                : Selector.SECOND);
                                                        tag = (daction == ItemLogger.ITEM_CREATE
                                                                ? Color.GREEN + "+"
                                                                : Color.RED + "-");
                                                    } else if (daction == ItemLogger.ITEM_SELL
                                                            || daction == ItemLogger.ITEM_BUY) { // LOOKUP_TRADE
                                                        selector = (daction == ItemLogger.ITEM_BUY
                                                                ? Selector.FIRST
                                                                : Selector.SECOND);
                                                        tag = (daction == ItemLogger.ITEM_BUY
                                                                ? Color.GREEN + "+"
                                                                : Color.RED + "-");
                                                ...
                                                for (String[] data : lookupList) {
                                                    ...
                                                    if (daction == 3 && !finalArgAction.contains(11) && amount == -1) {
                                                        if (dtype == 0) {
                                                            if (ConfigHandler.playerIdCacheReversed.get(ddata)
                                                                    == null) {
                                                            ...
                                                            dname = Util.getEntityType(dtype)
                                                                    .name();
                                                        }
                                                    } else {
                                                        dname = Util.getType(dtype)
                                                                .name()
                                                    ...
                                                    if (finalArgAction.contains(4)
                                                            || finalArgAction.contains(5)
                                                        ...
                                                        if (daction == 2 || daction == 3) {
                                                            ...
                                                            selector =
                                                                    (daction != 3 ? Selector.FIRST : Selector.SECOND);
                                            ...
                                            if (rows > displayResults) {
                                                ...
                                                if (finalArgAction.contains(6)
                                                        || finalArgAction.contains(7)
                                                        || finalArgAction.contains(9)
                                                        || (finalArgAction.contains(4)
                                                                && finalArgAction.contains(11))) {
